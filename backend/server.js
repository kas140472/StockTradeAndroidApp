...
app.post('/api/todoapp/adddata', jsonParser, async (request, response) => {
  try {
...
app.post('/api/todoapp/addportdata', jsonParser, async (request, response) => {
  ...
  console.log("server.js BUYoRsELL: ", buyOrSell);
  ...
  try {
    ...
    if (existingPortfolioItem) {
      ...
      if(buyOrSell == 'buy')
      {
        console.log("in buy: quantityData ", quantityData);
        updatedQuantity = existingPortfolioItem.quantityData + quantityData;
        console.log("in buy: updatedQuantity ", updatedQuantity);
        updatedTotalCost = existingPortfolioItem.totalCostData + totalCostData;
      }
      else {
        console.log("in sell: quantityData ", quantityData);
        updatedQuantity = existingPortfolioItem.quantityData - quantityData;
        console.log("in sell: updatedQuantity ", updatedQuantity);
        updatedTotalCost = existingPortfolioItem.totalCostData - totalCostData;
      }   
...
app.post('/api/todoapp/updatewalletdata',jsonParser, async (request, response) => {
  try {
    ...
    if (result.modifiedCount === 1) {
      response.status(201).json({ message: 'Wallet data successfully' });
    } else {
      response.status(201).json({ message: 'No Document added' });
    }
  } catch (error) {
...
app.delete('/api/todoapp/deletenotes', multer().none(),(request,response)=>{
  ...
  app.get('/company-news', async (req, res) => {
    console.log('Request received for /company-news');
  ...
  app.get('/company-eps', async (req, res) => {
    console.log('Request received for /company-eps');
    const symbol = req.query.symbol; // Get the symbol from the query parameter
    ...
    try {
      ...
      console.error('Error fetching company eps:', error.response?.data || error.message);
      res.status(error.response?.status || 500).send(error.response?.data || 'Internal Server Error');
    }
  });
  
  app.get('/company-rec', async (req, res) => {
    console.log('Request received for /company-rec');
    const symbol = req.query.symbol; // Get the symbol from the query parameter
    ...
    try {
      ...
      console.error('Error fetching company rec:', error.response?.data || error.message);
      res.status(error.response?.status || 500).send(error.response?.data || 'Internal Server Error');
    }
  });

  app.get('/company-insider', async (req, res) => {
    console.log('Request received for /company-insider');
    const symbol = req.query.symbol; // Get the symbol from the query parameter
    ...
    try {
      ...
      console.error('Error fetching company insider:', error.response?.data || error.message);
      res.status(error.response?.status || 500).send(error.response?.data || 'Internal Server Error');
    }
  });
